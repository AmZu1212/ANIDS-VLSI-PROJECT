...
from Verification.numeric_checks.numeric_check_cases import generate_all_cases
...
def main() -> int:
    ...
    hidden_out = _run_tb("Verification/numeric_checks/tb/hidden_stage_tb.sv")
    output_out = _run_tb("Verification/numeric_checks/tb/output_stage_tb.sv")
    loss_out = _run_tb("Verification/numeric_checks/tb/loss_stage_tb.sv")
    core_out = _run_tb("Verification/numeric_checks/tb/core_stage_tb.sv")
    ...
    for case in loss_cases:
        ...
        if rtl != case.expected_loss:
            print(f"LOSS_MISMATCH {case.name}: rtl={rtl} reference={case.expected_loss}")
            failed = True

    for case in core_cases:
        rtl = core_results.get(case.name)
        if rtl != (case.expected_loss, case.expected_outlier):
            print(f"CORE_MISMATCH {case.name}: rtl={rtl} reference={(case.expected_loss, case.expected_outlier)}")
            failed = True
    ...
    if failed:
        print("NUMERIC_CHECKS_FAILED")
        return 1

    print("NUMERIC_CHECKS_PASSED")
    return 0
