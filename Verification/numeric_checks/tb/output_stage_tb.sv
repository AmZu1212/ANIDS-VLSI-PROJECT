...
module output_stage_tb;
	localparam integer CASE_COUNT = 30;
	localparam string GENERATED_DIR = "Verification/numeric_checks/generated/output";
	...
	begin
		...
		for (neuron = 0; neuron < 128; neuron = neuron + 1) begin
			if (ready[neuron] !== 1'b1) begin
				$error("Output stage ready failure case=%0d neuron=%0d", idx, neuron);
				$finish;
			end
		end
