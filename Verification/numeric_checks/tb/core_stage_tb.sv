...
module core_stage_tb;
	localparam integer CASE_COUNT = 10;
	localparam string GENERATED_DIR = "Verification/numeric_checks/generated/core";
	...
	begin
		apply_reset;
		prog_file = $sformatf("%0s/core_%03d.prog", GENERATED_DIR, idx);
		vector_file = $sformatf("%0s/core_%03d.vector", GENERATED_DIR, idx);
		...
		fork
			...
			begin
				...
				#2;
				$display(
					"RTL_CORE case=core_%03d loss=%0d outlier=%0b",
					idx,
	...
	begin
		fd = $fopen(fname, "r");
		if (fd == 0)
			$fatal(1, "Cannot open core program file: %0s", fname);
		while (!$feof(fd)) begin
			rc = $fscanf(fd, "%h %h\n", addr, data);
			if (rc == 2)
	...
	task dma_send_vector_file_repeat(input string fname, input integer repeat_count);
		integer fd;
		integer idx;
	...
	begin
		fd = $fopen(fname, "r");
		if (fd == 0)
			$fatal(1, "Cannot open core vector file: %0s", fname);
		if ($fscanf(fd, "%h\n", vec) != 1)
			$fatal(1, "Bad core vector format: %0s", fname);
		$fclose(fd);
